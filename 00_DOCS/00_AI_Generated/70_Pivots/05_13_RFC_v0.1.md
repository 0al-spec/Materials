...
## Authors' Addresses / Contact Information
  ...
  * **Egor Merkushev**
      ...
      * Email: egor.merkushev@yandex.ru
    
## Status of this Memo

This document is an **Experimental** Request for Comments. It represents a draft proposal for an "AI Agent Passport" specification and is submitted to the community for discussion, feedback, critique, and suggestions for improvement. This document is a "work in progress" and is not yet a finalized standard. Distribution of this memo is unlimited.
...
To contribute to this document, please submit an issue or pull request to the original [GitHub repository](https://github.com/your-repo-here).
...
## Change Log / Version History
  ...
  * **Version 0.1 (2025-05-29):** Initial public draft of the AI Agent Passport Specification.
  * *(Future versions will detail changes here.)*
...
The Agent Passport is a central and integral concept within the Agentification Layer (AL) project. While AL was initially positioned as an execution environment (`agentifyd`), the Agent Passport specification and its associated tooling are intended to become the foundational open agent specification — with a strong emphasis on identity, integrity, and security. It defines the core trust framework that enables secure interaction, authentication, and delegation between agents in decentralized and hybrid environments. The `agentifyd` runtime will serve as a reference implementation of this standard, but the goal is to promote widespread adoption of the Agent Passport specification by other AI agent runtimes and frameworks (e.g., LangChain, AIOS, AutoGen) and cloud providers. AL aims to become the "standard keeper" and a key contributor to the development of this standard, promoting it through communities and organizations.
...
### 3.9. Other relevant terms
  ...
  * **Agentification Layer (AL):** Originally designed as an execution environment (`agentifyd`), the project has evolved to center around the Agent Passport specification and its associated tooling as its core focus. AL aspires to be the canonical authority for the Agent Passport standard, fostering its adoption across diverse AI agent platforms and frameworks.
...
This section specifies the system resources and external access that the agent requires to operate, facilitating proper environment provisioning and policy enforcement.
  ...
  * `resources` (object, optional):
      ...
      * `network` (object, optional): Network access requirements.
          * `outbound` (array of objects, optional): List of allowed outbound connections[cite: 163]. Each object[cite: 164]:
              * `protocol` (string, required): `tcp`, `udp`, `http`, `https`[cite: 164].
...
The declarative security policies embedded within the Agent Passport are crucial for runtime risk mitigation:
  ...
  * Network Security: This involves controlling an agent's ability to communicate over the network. By declaring allowed inbound and outbound connections (e.g., specific IP addresses, domains, ports, and protocols) in the passport's `networkRestrictions` or `resources.network` sections, unauthorized network access, data exfiltration, or participation in distributed attacks can be prevented.
...
The Agent Passport employs a two-tiered versioning strategy:
  ...
  * **Passport Versioning (`metadata.version`):** This field (e.g., `metadata.version: "1.0.0"`) denotes the specific version of an individual agent's passport definition[cite: 272]. This allows iteration on an agent's definition without changing the API version[cite: 273, 274].
...
**9.4. Auditing and Compliance**
  ...
  * Enhanced Accountability [cite: 291, 292]
  * Simplified Auditing [cite: 293, 294]
  * Demonstrable Compliance [cite: 295]

**10. IANA Considerations**

This section is reserved for considerations regarding registrations with the Internet Assigned Numbers Authority (IANA)[cite: 296]. While this is a draft specification, future versions of this RFC may request the registration of a media type (e.g., `application/vnd.agent-passport+yaml`) or a specific file extension for Agent Passport files[cite: 34, 35, 297]. Additionally, considerations for URI schemes related to Agent Passports or Issuing Authorities might arise[cite: 34]. These will be detailed in future iterations pending further development and community consensus[cite: 297].

**11. Acknowledgements**

[RFC Editor: This section can be populated before publication.]
...
The authors would like to thank the following individuals and organizations for their contributions to the ideas, discussions, and reviews during the development of this specification (even at these early stages):
  ...
  * *(List individuals/organizations here, if any at this stage.)* [cite: 28, 29]
...
**12.1. Normative References**
  ...
  * **YAML Specification:** The official specification for YAML Ain't Markup Language (YAML™) Version 1.2.2[cite: 298].
  * **RFC 2119:** S. Bradner, "Key words for use in RFCs to Indicate Requirement Levels," March 1997[cite: 299].
  * **RFC 8174:** B. Leiba, "Ambiguity of OAUTH when applied to security protocols," May 2017 (clarifies RFC 2119 usage)[cite: 300].
  * **Cryptographic Signature Standards:** Relevant industry standards for digital signatures (e.g., RSASSA-PSS, EdDSA) and hashing algorithms (e.g., SHA-256) as defined by NIST or other recognized bodies[cite: 301].

**12.2. Informative References**

  * **A2A Protocol:** Reference to existing agent-to-agent communication protocols, specifically where the "Agent Card" concept is introduced[cite: 302]. (Specific URL/RFC to be added if available, otherwise refers to the general concept)[cite: 303].
  * **Decentralized Identifiers (DIDs) v1.0:** W3C Recommendation. (e.g., [https://www.w3.org/TR/did-core/](https://www.w3.org/TR/did-core/))[cite: 303].
  * **Verifiable Credentials Data Model v1.0:** W3C Recommendation. (e.g., [https://www.w3.org/TR/vc-data-model/](https://www.w3.org/TR/vc-data-model/))[cite: 304].
  * **seccomp (Secure Computing Mode):** Linux kernel feature for syscall filtering[cite: 305].
  * **chroot:** Unix operation to change the apparent root directory[cite: 306].
  * **Linux Capabilities:** System for partitioning superuser privileges[cite: 307].
  * **Agentification Layer (AL) Project Documentation:** Internal or public documentation for the AL project, agentifyd runtime, and AgentHub[cite: 308].

**Appendix A. Comparison with A2A Agent Card**

The Agent Passport builds upon, and significantly extends, concepts found in simpler agent description mechanisms, such as the A2A Agent Card[cite: 309]. Key differences are highlighted below[cite: 310, 311]:
...
This appendix outlines areas for future work and topics requiring further discussion[cite: 313]:

  * Standardization Body Engagement [cite: 314]
  * Registry for Issuing Authorities [cite: 315]
  * Advanced Policy Expressions [cite: 316]
  * Telemetry and Monitoring Integration [cite: 317]
  * Attestation for Agent Behaviors [cite: 318]
  * Privacy Considerations [cite: 319]
  * Interoperability with Existing Identity Systems (IAM, OAuth 2.0, OpenID Connect) [cite: 320]
  * Formal Verification [cite: 321]
  * Decentralized Storage [cite: 322]
  * Economic Models [cite: 323]